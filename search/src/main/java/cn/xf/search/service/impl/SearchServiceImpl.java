package cn.xf.search.service.impl;

import cn.xf.common.es.SkuEsModel;
import cn.xf.common.utils.R;
import cn.xf.search.config.ElasticSearchConfig;
import cn.xf.search.constant.EsConstant;
import cn.xf.search.feign.ProductFeign;
import cn.xf.search.service.SearchService;
import cn.xf.search.vo.AttrResponseVo;
import cn.xf.search.vo.BrandVo;
...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("http://search.mall.xyz/list.html?" + replace);
                return navVo;
        ...
        if (param.getBrandId() != null && param.getBrandId().size() > 0) {
            ...
            navVo.setNavName("品牌");
            // 远程获取品牌信息---根据品牌id查询品牌信息
            R brandInfosByBrandIds = productFeign.getBrandInfosByBrandIds(param.getBrandId());
            if (brandInfosByBrandIds.getCode() == 0) {
                List<BrandVo> brandVoList = brandInfosByBrandIds.getData("result", new TypeReference<List<BrandVo>>() {
                });
                StringBuffer buffer = new StringBuffer();
                String replace = "";
                for (BrandVo item : brandVoList) {
                    buffer.append(item + ";");
                    replace = replaceQueryString(param, item.getBrandId() + "", "brandId");
                }
    ...
    private SearchRequest buildSearchRequest(SearchParam param) {
        ...
        // 1.1 bool-must，模糊匹配
        // StringUtils.isEmpty过时，使用isEmpty代替
        if (!StringUtils.isEmpty(param.getKeyword())) {
            boolQueryBuilder.must(QueryBuilders.matchQuery("skuTitle", param.getKeyword()));
        }

        //1.2 bool-filter
        // 1.2.1 按照三级分类Id查询---termQuery
        if (null != param.getCatalog3Id()) {
            boolQueryBuilder.filter(QueryBuilders.termQuery("catalogId", param.getCatalog3Id()));
        }

        //1.2.2 按照品牌Id（集合）查询---termsQuery
        ...
        if (!StringUtils.isEmpty(param.getSkuPrice())) {
            //skuPrice形式为：1_500（1~500）或_500（<500）或500_（>500）
